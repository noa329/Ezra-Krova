  ...
  template: `
    <div class="auth-page">
      <mat-card class="auth-card">
        <mat-card-header><mat-card-title>כניסה למערכת</mat-card-title></mat-card-header>
        <mat-card-content>
          ...
          <p class="link-text">אין לך חשבון? <a routerLink="/register">הירשם עכשיו</a></p>
        </mat-card-content>
      </mat-card>
    </div>
  `,
...
export class LoginComponent {
  ...
  onSubmit() {
    ...
    this.auth.login(this.form.value as any).subscribe({
      ...
      error: (err) => { this.error = err.error?.message || 'שגיאה בכניסה'; this.loading = false; },
    });
  }
}
